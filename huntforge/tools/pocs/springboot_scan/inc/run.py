...
from inc import output, console, policy
...
DIR_WORDLIST = TOOL_ROOT / "Dir.txt"
...
def url(urllist, proxies, header_new, *, delay=0.0, output_file):
    output_path = Path(output_file)
    output_path.write_text("", encoding="utf-8")
    cprint(f"======开始对目标URL测试SpringBoot信息泄露端点======", "cyan")
    encountered_hashes = []
    findings = []
    with DIR_WORDLIST.open('r', encoding="utf-8") as web:
        webs = web.readlines()
        for web in webs:
            web = web.strip()
            u = urllist + web
    ...
    if count >= 1:
        print('\n')
        cprint("[+][+][+] 发现目标URL存在SpringBoot敏感信息泄露，已写入显式输出文件，共%d行记录" % count, "red")
    else:
        print('\n')
...
async def async_dir(url, proxies, header_new, semaphore, delay, output_file, output_lock):
    try:
        tasks = []
        u_list = []
        with DIR_WORDLIST.open('r', encoding="utf-8") as web:
        ...
        cprint(f"[-] 安全扫描错误: {type(e).__name__}", "yellow")

async def file(u, proxies, header_new, output_file, output_lock):
    try:
        header = {"User-Agent": random.choice(ua)}
        newheader = json.loads(str(JSON_handle(header, header_new)).replace("'", "\""))
        # 处理代理格式
...
async def file_semaphore(url, proxies, header_new, semaphore, delay, output_file, output_lock):
    async with semaphore:
        await file(url, proxies, header_new, output_file, output_lock)
    ...
    *,
    ...
    while True:
        try:
            ...
            break
    count = len(output_path.read_text(encoding="utf-8").splitlines())
    if count >= 1:
        print('\n')
        cprint("[+][+][+] 发现目标TXT内存在SpringBoot敏感信息泄露，已写入显式输出文件，共%d行记录" % count, "red")
    else:
        print('\n')
...
def dump(urllist, proxies, header_new, *, artifact_dir):
    ...
    def download(url: str, fname: str, proxies: str, newheader):
        ...
        output_path = policy.artifact_path(artifact_dir, fname)
    ...
    try:
        ...
        cprint(f"[-] 下载失败: {type(e).__name__}", "yellow")
        policy.append_artifact(artifact_dir, "error.log", str(e) + '\n')
        sys.exit()

def dumpfile(input_file, proxies, header_new, *, artifact_dir, delay=0.0):
    artifact_dir = policy.resolve_artifact_dir(artifact_dir)
    header = {"User-Agent": random.choice(ua)}
    newheader = json.loads(str(JSON_handle(header, header_new)).replace("'", "\""))
        ...
        try:
            for path in paths:
                ...
                try:
                    ...
                    print(f"[-] 访问 {full_url} 出现错误: {type(e).__name__}")
                    raise
        except Exception as e:
            print(f"[.] 正在跳过该报错URL")
            continue
    
    dump_output = policy.artifact_path(artifact_dir, "dumpout.txt")
    with dump_output.open('w', encoding="utf-8") as f:
        for valid_url in valid_urls:
            f.write(valid_url + '\n')
    count = len(dump_output.read_text(encoding="utf-8").splitlines())
    if count >= 1:
        print('\n')
        cprint("[+][+][+] 发现目标TXT内存在SpringBoot敏感文件泄露，已写入本次运行产物目录，共%d行记录" % count, "red")
    else:
